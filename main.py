...
class KMapWidget(Widget):
    ...
    def draw(self):
        self.canvas.before.remove_group("kmap_grid")
        ...
        ox, oy, otop = self.x, self.y, self.top

        # Cell backgrounds + borders go in canvas.before (with a group id so we
        # can clear just this part on redraw) so they render BEHIND the value
        # labels added as child widgets below - otherwise the labels get
        # painted over and become invisible.
        with self.canvas.before:
            for i, rv in enumerate(rows):
                for j, cv in enumerate(cols):
                    x = ox + left + j * cw
                    y = otop - top - (i + 1) * ch
                    Color(1, 1, 1, 1, group="kmap_grid")
                    Rectangle(pos=(x, y), size=(cw, ch), group="kmap_grid")
                    Color(*BORDER, group="kmap_grid")
                    Line(rectangle=(x, y, cw, ch), width=1.2, group="kmap_grid")

        self._draw_text_overlay(ox, oy, otop, left, top, cw, ch, rows, cols)
        ...
        for gi, term in enumerate(self.groups):
            ...
            with self.canvas.after:
                Color(*color)
                x = ox + left + c0 * cw + inset
                y = otop - top - (r1 + 1) * ch + inset
                w = (c1 - c0 + 1) * cw - inset * 2
                h = (r1 - r0 + 1) * ch - inset * 2
                Line(rounded_rectangle=(x, y, w, h, dp(10)), width=2.6)

    def _draw_text_overlay(self, ox, oy, otop, left, top, cw, ch, rows, cols):
